---
title: "Damage caused by weather events in the United States between 1996 and 2011"
output:
  md_document:
...
library(grid)
```
I read in the data using fread, which turned out to be the fastest solution for me. Next I remove all the columns except "BGN_DATE", "EVTYPE", "FATALITIES", "INJURIES", "PROPDMG", "PROPDMGEXP", "CROPDMG" and "CROPDMGEXP". Note that the libraries used for this report are commented here:
...
```{r substitute events, cache = TRUE}
...
```{r plot fatalities, fig.width = 10, fig.height = 8}
...
```{r plot injuries, fig.width = 10, fig.height = 8}
par(mar=c(9,4.1,4.1,2.1))
